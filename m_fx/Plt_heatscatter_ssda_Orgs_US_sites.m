...
figid     = {'(b)','(a)','(c)','(d)','(e)'};
nStd      = 0;
FigName   = ['./figs/xheatscatters_ssda_',EstOrg,'_vs_',StOrg,'_all_rm_cld'];         
if nStd == 0
    optOut = 'no';
else
    optOut = 'yes';   
...
%Estimation
%++++++++++
StLon       = [-116.018,-88.37,-89.97,-105.10,-105.237,-77.93,-96.62,-97.485];
StLat       = [  36.626, 40.06, 34.25,  48.31,  40.125, 40.72, 43.73, 36.605]	;
nSt         = num2str(length(StLat));
stid  		= {'dra', 'bon', 'gwn','fpk','tbl','psu','sxf','ARM/SGPC1'};
     
% ssda_SURFRAD_7st_daily_20131001_20150831.nc
%ssda_UMD_and_LIS3INSTAVGS_4_SURFRAD_7st_daily_20131001_20150831.nc
 estfname = [pth,'ssda_',EstOrg,'_4_SURFRAD_7st_daily_20131001_20150831_ERA5redownloaded.nc'];
 %ncdisp(estfname)
 estdata  = ncread(estfname,'ssda');
 estdata(estdata<=0)=NaN;
%  whos estdata
 %estdata(1,1,1:10)
 estfname1 = [pth,'ssda_',EstOrg,'_4_ARM_1st_daily_20131001_20150831.nc'];
 estdata1  = ncread(estfname1,'ssda');
 estdata1(estdata1<=0)=NaN;

%  whos estdata1
 estdata(1:5,8,1:700)=estdata1;
%  whos estdata
 %estdata(1,1,1:10)
...
%  whos stdata
 ...
 stfname1  = [pth,'ssda_ARM_SGP_C1_daily_20131001_20150831.dat']; 
 fid      = fopen(stfname1,'r');
 stdata1	  = fread(fid,nt,'float');
 stdata1(stdata1<0)=NaN;
 fclose(fid);
%  whos stdata1
 stdata(8,1:700)=stdata1(:);
%  whos stdata
...
%                 [sprintf('Bias=%.2f', Bias), '(',num2str(abs(Bias/avg)*100,'%.2f'),'%)'],...
%                 [sprintf('Std=%.2f', Std),'(',num2str(abs(Std/avg)*100,'%.2f'),'%)'],...
%                  [sprintf('RMSE=%.2f', RMSE),'(',num2str(abs(RMSE/avg)*100,'%.2f'),'%)'],...
%                  sprintf('N=%d', N)};
 %disp(statistics)
 %EstOrgNames{iorg},
statistics =  {sprintf('R=%.2f', Corr),sprintf('N=%d', N)...
                [sprintf('Bias=%.2f', Bias), '(',num2str(abs(Bias/avg)*100,'%.2f'),'%)'],...
                 [sprintf('RMSE=%.2f', RMSE),'(',num2str(abs(RMSE/avg)*100,'%.2f'),'%)'],...
                 };
...
cap1=annotation('textbox', [0.15 0.85 0.1 0.1],...
 'String',statistics(1:2),...
  'EdgeColor', 'none', 'FontSize', 18); 
cap11=annotation('textbox', [0.285 0.85 0.1 0.1],...
 'String',statistics(3:end),...
  'EdgeColor', 'none', 'FontSize', 18); 
cap2=annotation('textbox', [0.8 0.125 0.1 0.1],...
 'String',figid(iorg),...
  'EdgeColor', 'none', 'FontSize', 18); 
cap22=annotation('textbox', [0.45 0.125 0.1 0.1],...
 'String',EstOrgNames(iorg),...
  'EdgeColor', 'none', 'FontSize', 18);

ax1=gca;
% set(ax1.Colorbar,'Limits',[1 14]);
...
%         scatter_COL(i) = values(id_X, id_Y);
        scatter_COL(i) = values(id_X, id_Y)/onepercent;
